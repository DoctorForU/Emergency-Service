...
public class EmergencyService { // 응급실 실시간 가용병상정보 조회 -> 응급실 데이터
    ...
    private String buildApiUrl(EmergencyRequest request) { // api 호출 url 만들기
        ...
        if (request.getSTAGE1() != null && !request.getSTAGE1().isEmpty()) {
            apiUrl.append("&STAGE1=").append(encodeValue(request.getSTAGE1()));
        }
        if (request.getSTAGE2() != null && !request.getSTAGE2().isEmpty()) {
            apiUrl.append("&STAGE2=").append(encodeValue(request.getSTAGE2()));
        }
        ...
        try {
            ...
            if (items.isArray()) {
                for (JsonNode item : items) {
                    ...
                    emergencyResponse.setHvidate(item.path("hvidate").asText());
                    emergencyResponse.setHvdnm(item.path("hvdnm").asText());
                    emergencyResponse.setHv1(item.path("hv1").asText());
                    emergencyResponse.setDutyname(item.path("dutyname").asText());
                    emergencyResponse.setDutytel3(item.path("dutytel3").asText()); // 기본정보란

                    // 실시간 가용 가능 장비////////////////////////
                    emergencyResponse.setHvctayn(item.path("hvctayn").asText());
                    emergencyResponse.setHvmriayn(item.path("hvmriayn").asText());
                    emergencyResponse.setHvangioayn(item.path("hvangioayn").asText());
                    emergencyResponse.setHvventiayn(item.path("hvventiayn").asText());
                    emergencyResponse.setHvventisoayn(item.path("hvventisoayn").asText());
                    emergencyResponse.setHvincuayn(item.path("hvincuayn").asText());
                    emergencyResponse.setHvcrrtayn(item.path("hvcrrtayn").asText());
                    emergencyResponse.setHvecmoayn(item.path("hvecmoayn").asText());
                    emergencyResponse.setHvoxyayn(item.path("hvoxyayn").asText());
                    emergencyResponse.setHvhypoayn(item.path("hvhypoayn").asText());
                    emergencyResponse.setHvamyn(item.path("hvamyn").asText());
                    ...
                    emergencyResponses.add(emergencyResponse);
                }
            }
        } catch (Exception e) {
